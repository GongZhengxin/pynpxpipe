...
_DEFAULT_SORTING = SortingConfig(
    ...
    analyzer=AnalyzerConfig(
        random_spikes=None,  # type: ignore[arg-type]
        waveforms=None,  # type: ignore[arg-type]
        template_operators=["similarity", "amplitude"],
    ),
)
...
class SortingForm:
    ...
    def __init__(self, state: AppState) -> None:
        ...
        self.sorter_select = pn.widgets.Select(
            name="Sorter", options=_SORTER_OPTIONS, value="kilosort4"
        )
        self.mode_select = pn.widgets.Select(name="Mode", options=_MODE_OPTIONS, value="local")
        self.nblocks_input = pn.widgets.IntInput(
            name="nblocks", value=_DEFAULT_PARAMS.nblocks, start=0
        )
        self.th_learned_input = pn.widgets.FloatInput(
            name="Th_learned", value=_DEFAULT_PARAMS.Th_learned, step=0.5
        )
        self.do_car_checkbox = pn.widgets.Checkbox(name="do_CAR", value=_DEFAULT_PARAMS.do_CAR)
        self.batch_size_input = pn.widgets.TextInput(
            name="batch_size", value=str(_DEFAULT_PARAMS.batch_size)
        )
        self.n_jobs_input = pn.widgets.IntInput(
            name="n_jobs", value=_DEFAULT_PARAMS.n_jobs, start=1
        )
        self.import_path_input = pn.widgets.TextInput(
            name="Import Path", placeholder="/path/to/kilosort_output", visible=False
        )
    ...
    def _rebuild_config(self, event=None) -> None:
        ...
        self._state.sorting_config = SortingConfig(
            ...
            analyzer=_DEFAULT_SORTING.analyzer,
        )
