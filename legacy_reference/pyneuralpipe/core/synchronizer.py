...
class DataSynchronizer:
    ...
    def _check_ml_ni_alignment(self):
        ...
        try:
            ...
            # 确保每个试次的 code 是数组格式
            def safe_count(codes, target):
                codes = np.atleast_1d(codes) if not isinstance(codes, (list, np.ndarray)) else np.asarray(codes)
                return np.sum(codes == target)
            
            self.onset_times_ml = np.sum([safe_count(ML_code, stim_onset_code) for ML_code in ML_codenumbers])
            offset_times = np.sum([safe_count(ML_code, stim_offset_code) for ML_code in ML_codenumbers])
            
            onset_times_by_trial_ML = np.array([safe_count(ML_code, stim_onset_code) for ML_code in ML_codenumbers])
    ...
    def _validate_eye_tracking(self):
        ...
        try:
            ...
            for i_trial in range(num_trial):
                ...
                stim_ondur = int(ML_stimons[i_trial])  # 确保是整数
                
                # 确保 codenumbers 是数组格式（防御性处理）
                trial_codes = ML_codenumbers[i_trial]
                trial_codes = np.atleast_1d(trial_codes) if not isinstance(trial_codes, (list, np.ndarray)) or (isinstance(trial_codes, np.ndarray) and trial_codes.ndim == 0) else np.asarray(trial_codes)
                
                stim_onset_loc = np.where(trial_codes == 64)[0]
                if len(stim_onset_loc) == 0:
                    continue
                
                # 确保 codetimes 也是数组格式
                trial_times = ML_codetimes[i_trial]
                trial_times = np.atleast_1d(trial_times) if not isinstance(trial_times, (list, np.ndarray)) or (isinstance(trial_times, np.ndarray) and trial_times.ndim == 0) else np.asarray(trial_times)
                ...
                for stim_idx in range(len(stim_onset_loc)):
                    stim_start_time = trial_times[stim_onset_loc[stim_idx]]
                    ...
                    if eye_data.shape[1] == 2:
                        actual_len = min(eye_data.shape[0], stim_ondur)
                        eye_matrix[stim_global_idx, 0:actual_len, :] = eye_data[:actual_len, :]
                    else:
                        self.logger.log_warning(f"试次{i_trial}: eye_data形状异常 {eye_data.shape}, 期望第二维为2")
                    stim_global_idx += 1
